...
export default async function SellersPage() {
  const [sellers, cities] = await Promise.all([
    prisma.seller.findMany({
      include: { city: true, _count: { select: { items: true } } },
        ...
        <form action={createSeller} className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <p className="text-sm text-gray-600 mb-2">
              Ставка за товар по категориям (₽):
            </p>
            <div className="grid grid-cols-4 gap-2">
              <input
                name="rate_electronics"
                type="number"
                step="0.01"
          ...
          <tbody>
            {sellers.map((s) => (
              <tr key={s.id} className="border-t">
                ...
                <td className="p-3">{s.city.name}</td>
                <td className="p-3 text-xs">
                  {s.ratesJson
                    ? Object.entries(s.ratesJson as Record<string, number>)
                        .map(([k, v]) => `${k}: ${v}₽`)
                        .join(", ")
                    : "—"}
                </td>
